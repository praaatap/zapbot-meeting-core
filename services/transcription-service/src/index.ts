...
import { transcriptionConsumer } from './workers/sqs.consumer.js';
...
const start = async () => {
  try {
    ...
    transcriptionConsumer.start();
    logger.info('Transcription Consumer started');

    // 3. Start HTTP server for health/metrics
    app.listen(PORT, () => {
      logger.info(`Transcription Service HTTP server started on port ${PORT}`);
    });
  } catch (error) {
...
process.on('SIGTERM', async () => {
  ...
  await transcriptionConsumer.stop();
